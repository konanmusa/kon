...
g= datetime.datetime(2023, 10, 22, 0, 0 ,0)
'''
...
if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   if(x.strftime("%X"))>(g.strftime("%X")):
    print('\n\n')
    print("     "+'خلص اشتراك عليك مراجعة صانع اداة https://t.me/hkeraldola🗿')
    print('\n\n')
    print(x)
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [ KONAN ] ㅅ %s/%s ㅅ OK:%s ㅅ CP:%s ㅅ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
